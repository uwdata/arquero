...
import { BitSet, Table, not } from '../../src/index.js';
import { filter, groupby, orderby } from '../../src/verbs/index.js';
...
describe('Table', () => {
  ...
  it('supports column values output', () => {
    ...
    const dt = orderby(filter(t, d => d.v > 1), 'v');
  ...
  it('supports grouped object output', () => {
    ...
    assert.deepEqual(
      groupby(dt, 'u').objects({ grouped: 'object' }),
      {
        a: [
          { u: 'a', v: 1 },
          { u: 'a', v: 2 },
    ...
    assert.deepEqual(
      groupby(dt, 'u').objects({ grouped: 'entries' }),
      [
        ['a',[
          { u: 'a', v: 1 },
          { u: 'a', v: 2 },
    ...
    assert.deepEqual(
      groupby(dt, 'u').objects({ grouped: 'map' }),
      new Map([
        ['a',[
          { u: 'a', v: 1 },
          { u: 'a', v: 2 },
    ...
    assert.deepEqual(
      groupby(dt, 'u').objects({ grouped: true }),
      new Map([
        ['a',[
          { u: 'a', v: 1 },
          { u: 'a', v: 2 },
    ...
    assert.deepEqual(
      groupby(dt, 'u').objects({ limit: 3, grouped: 'object' }),
      {
        a: [
          { u: 'a', v: 1 },
          { u: 'a', v: 2 }
    ...
    assert.deepEqual(
      groupby(dt, 'u').objects({ offset: 2, grouped: 'object' }),
      {
        a: [
          { u: 'a', v: 4 }
        ],
    ...
    assert.deepEqual(
      groupby(dt2, ['u', 'w']).objects({ grouped: 'object' }),
      {
        a: {
          x: [{ u: 'a', w: 'x', v: 1 }],
    ...
    assert.deepEqual(
      groupby(dt2, ['u', 'w']).objects({ grouped: 'entries' }),
      [
        ['a', [
          ['y', [{ u: 'a', w: 'y', v: 2 }, { u: 'a', w: 'y', v: 4 }]],
          ['x', [{ u: 'a', w: 'x', v: 1 }]]
    ...
    assert.deepEqual(
      groupby(dt2, ['u', 'w']).objects({ grouped: 'map' }),
      new Map([
        ['a', new Map([
          ['x', [{ u: 'a', w: 'x', v: 1 }]],
          ['y', [{ u: 'a', w: 'y', v: 2 },{ u: 'a', w: 'y', v: 4 }]]
